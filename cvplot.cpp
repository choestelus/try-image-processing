...
Series::Series(const Series& s):count(s.count), label(s.label), auto_color(s.auto_color), color(s.color)
{
	...
	memcpy(data, s.data, count * sizeof(double));
}
...
void Series::SetData(int n, double *p)
{
	Clear();
...
void Figure::Initialize()
{
	...
	x_scale = 1.0f;
	if (x_max - x_min > 1)
		x_scale = (double)(figure_size.width - border_size * 2) / (x_max - x_min);

	y_scale = (double)(figure_size.height - border_size * 2) / y_range;
}
...
void PlotManager::Plot(const string figure_name, const double *p, int count, int step,
					   int R, int G, int B)
{
	if (count < 1)
		return;
	...
	double *data_copy = new double[count];
		  ...
		  int R, int G, int B)
{
	...
	double  *data_copy = new double[count * step];

	double   *dst = data_copy;
	const T *src = p;
...
template
void plot(const string figure_name, const unsigned short* p, int count, int step,
		  int R, int G, int B);

template
void plot(const string figure_name, const unsigned int* p, int count, int step,
		  int R, int G, int B);
